...
def mesage_locate(chatPart, message):
    if message != '':
        ...
        if chatPart.index('end') != None:
            ...
            chatPart.tag_add("YOU", LineNumber, LineNumber + 0.4)
            chatPart.config(state=tk.DISABLED)
            chatPart.yview(tk.END)


def config_rec_message(chatPart, message):
    if message != '':
        chatPart.config(state=tk.NORMAL)
        if chatPart.index('end') != None:
            try:
...
class GUI:
    def __init__(self, connection, ip, protocol_option):
        ...
        self.send_butt = tk.Button(self.chat, font=("Consoles", 10), text=u"SEND", width="40", height=5,
                                   bd=0, activebackground="#BDE096", justify="center",
                                   command=self.actionOnClick)
        self.sound_butt = tk.Button(self.chat, font="Consoles", text="\uD834\uDD60", width="40", height=5,
                                    bd=0, activebackground="#BDE096", justify="center",
                                    command=self.sound_list)

        self.textBox = tk.Text(self.chat, bd=0, width="20", height="5", font="Consoles")
    ...
    def download_file_function(self):
        ...
        if self.protocol == "tcp":
            self.client_socket.sendall("mess".encode("utf-8"))
            tcpreceiver.tcp_connect(path)
        elif self.protocol == "udp":
            self.client_socket.sendto("mess".encode("utf-8"), (self.client_ip, self.server_port))
    ...
    def sending_func(self):
        if self.protocol == 'tcp':
            self.file_tcp_sender = tcpsender.tcp_sending(self.client_ip, self.path)
        else:
            self.file_udp_sender = udpsender.udp_sender(self.client_ip, self.path)
            ...
            while 1:
                ...
                if data != '':
                    ...
                    if data1 == True:  # if we have a file
                        ...
                        self.receiving(name)
                        config_rec_message(self.chatPart, name2)
                    else:
                        config_rec_message(self.chatPart, data1)
    ...
    def actionOnClick(self):
        ...
        if self.protocol == "tcp":
            self.client_socket.sendall(messageText.encode("utf-8"))
        else:
            print(self.client_ip, self.server_port)
            self.client_socket.sendto(messageText.encode("utf-8"), (self.client_ip, self.server_port))
    ...
    def sound_list(self):
        ...
        self.button_list = [i for i in range(len(sounds))]
        for i, j in sounds.items():
            self.button_list[i] = tk.Button(self.sound_list_box, font="Consoles", text=j, bd=0, activebackground="#BDE096",
                                            justify="left", command=partial(self.sound_index, i)).pack(
                side="left")
...
class ConnectionGUI(tk.Frame):
    def __init__(self, connection_part):
        ...
        tk.Label(self.connection_part, text="Project 2", bg='lavender', fg="black",
                 font=("Consoles", 15, "bold italic")) \
            .grid(column=0, row=0, sticky="WE", padx=0, columnspan=4)
        tk.Label(self.connection_part, text="Enter Destination IP: ", fg="black", bg='lavender', font=("Consoles", 10)) \
            .grid(column=0, row=1)

        self.ip = tk.Entry(self.connection_part)
        self.ip.insert(tk.END, '192.168.1.111')
        self.ip.grid(column=2, row=1)
        self.new_frame = tk.Frame(self.connection_part, highlightbackground="black", bg='lavender', highlightcolor="black",
                                  highlightthickness=2, width=280, height=100)
        self.new_frame.grid(row=2, column=0, columnspan=3, pady=10)
        self.new_frame.grid_propagate(False)

        tk.Label(self.new_frame, text="Protocol: ", fg="black", font=("Consoles", 10), bg='lavender') \
            .grid(column=0, row=2, columnspan=1)

        self.protocol_option = tk.StringVar()
        self.protocol_option.set('tcp')
        self.protocol_option_1 = tk.Radiobutton(self.new_frame, text="TCP", variable=self.protocol_option, bg='lavender',
                                                value="tcp").grid(column=1, row=2)
        self.protocol_option_2 = tk.Radiobutton(self.new_frame, text="UDP", variable=self.protocol_option, bg='lavender',
                                                value="udp").grid(column=2, row=2)

        self.submit_button = tk.Button(self.new_frame, text=u'Start Connection=>', command=self.waitConnection, bg='lavender',
                                       width=23)
        self.submit_button.grid(row=3, column=1, pady=10, columnspan=2)
        self.connection_part.mainloop()

    def waitConnection(self):
        self.label11 = tk.Label(self.connection_part, text="Waiting for Connection ", fg="red",
                                font=("Consoles", 12)).grid(column=2, row=4)
        get_ip = self.ip.get()
        port_num = 3000
    ...
    def start_connection(self, ip, port, protocol):
        ...
        if protocol == "tcp":
            ...
            GUI(client_socket, self.ip, protocol)
        elif protocol == "udp":
            client_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
            client_socket.connect((self.ip, self.port))
            client_socket.sendto("udp".encode("utf-8"), (self.ip, self.port))
            GUI(client_socket, self.ip, protocol)
        else:
